...
INDIRME_KLASORU = os.path.expandvars(r"%USERPROFILE%\Downloads")
...
def silent_shell(command, **kwargs):
    return silent_popen(["cmd", "/c", command], **kwargs)


def open_uri_silent(uri):
    try:
        os.startfile(uri)
        return True
    except Exception:
        try:
            silent_shell(f'start "" "{uri}"')
            return True
        except Exception:
            return False
...
class JarvisCore(QObject):
    ...
    def __init__(self):
        ...
        self.download_watch_stop = threading.Event()
        self.download_watch_seen = set()
        self.download_watch_started = False
    ...
    def log_shutdown(self):
        if hasattr(self, "download_watch_stop"):
            self.download_watch_stop.set()
        self.log_event("INFO", f"Jarvis kapatildi {APP_VERSION}")

    def downloads_folder(self):
        primary = Path(INDIRME_KLASORU)
        if not primary.exists():
            print("[GÜVENLİK] HATA: İndirme klasörü bulunamadı:", INDIRME_KLASORU)
            self.log_event("GÜVENLİK", f"HATA: İndirme klasörü bulunamadı: {INDIRME_KLASORU}")
        candidates = [
            primary,
            Path.home() / "Downloads",
            Path(os.getenv("USERPROFILE", str(Path.home()))) / "Downloads",
            Path(os.getenv("USERPROFILE", str(Path.home()))) / "İndirilenler",
            Path(os.getenv("USERPROFILE", str(Path.home()))) / "Indirilenler",
        ]
    ...
    def start_watchdog(self):
        if self.download_watch_started:
            return
        self.download_watch_started = True
        print("[GUVENLIK] Indirme takibi baslatiliyor...")
        self.log_event("GUVENLIK", "Indirme takibi gecikmeli baslatiliyor")
        threading.Thread(target=self.indirme_takip_baslat, daemon=True).start()

    def is_risky_download(self, path):
        suffix = Path(path).suffix.lower()
        return suffix in {".exe", ".msi", ".bat", ".cmd", ".ps1", ".vbs", ".js", ".scr", ".com", ".jar"}

    def handle_download_event(self, path):
        file_path = Path(path)
        if not self.is_risky_download(file_path):
            return
        key = str(file_path).lower()
        if key in self.download_watch_seen:
            return
        self.download_watch_seen.add(key)
        message = f"Efendim, İndirilenler klasörüne riskli bir dosya geldi: {file_path.name}. Açmadan önce virüs taraması yapmanızı öneririm."
        self.log_event("GÜVENLİK", f"Riskli indirme tespit edildi: {file_path}")
        self.log.emit(f"> GÜVENLİK: Riskli indirme tespit edildi: {file_path.name}")
        self.security_alert.emit(message)
        # Sesli uyari UI ana thread'inde guvenlik_uyari_goster ile verilir.

    def indirme_takip_baslat(self):
        downloads = self.downloads_folder()
        print("[GÜVENLİK] Watchdog observer başladı:", downloads)
        try:
            downloads.mkdir(parents=True, exist_ok=True)
        except Exception:
            pass

        self.log_event("GÜVENLİK", "İndirme takibi başlatıldı")
        self.log.emit("> GÜVENLİK: İndirme takibi başlatıldı")

        try:
            for existing in downloads.iterdir() if downloads.exists() else []:
                self.download_watch_seen.add(str(existing).lower())
        except Exception as exc:
            self.log_event("HATA", f"Indirme klasoru ilk tarama hatasi: {exc}")
        ...
        if Observer is not None and FileSystemEventHandler is not None:
            ...
            class DownloadHandler(FileSystemEventHandler):
                def on_created(self, event):
                    if event.is_directory:
                        return
                    core.handle_download_event(event.src_path)

                def on_moved(self, event):
                    if event.is_directory:
                        return
                    core.handle_download_event(event.dest_path)
            ...
            try:
                ...
                try:
                    observer.start()
                    print("[GÜVENLİK] Observer aktif")
                except Exception as e:
                    print("[GÜVENLİK] Observer hatası:", e)
                    raise
                while not self.download_watch_stop.is_set():
                    time.sleep(1)
                return
            except Exception as exc:
                self.log_event("HATA", f"Watchdog indirme takibi hata verdi: {exc}")
                self.log.emit(f"> GÜVENLİK HATASI: Watchdog hata verdi, polling deneniyor: {exc}")
            finally:
                try:
                    observer.stop()
        ...
        known = set(self.download_watch_seen)
        while not self.download_watch_stop.is_set():
            try:
                current = {str(path).lower(): path for path in downloads.iterdir()} if downloads.exists() else {}
                for key, path in current.items():
                    if key not in known:
                        known.add(key)
                        self.handle_download_event(path)
            except Exception as exc:
                self.log_event("HATA", f"Polling indirme takibi hata verdi: {exc}")
            time.sleep(3)

    def open_edge_private_urls(self, urls, gap=0.6):
        for url in urls:
            silent_shell(f'start msedge -inprivate "{url}"')
            time.sleep(gap)

    def open_opera_url(self, url, fullscreen=False, side_screen=False):
        silent_shell(f'start opera "{url}"')
    ...
    def open_spotify_app_search(self, query, autoplay=False):
        ...
        open_uri_silent(f"spotify:search:{urllib.parse.quote(query)}")
    ...
    def open_lol_helper(self, champion=None, role=None):
        ...
        return (
            f"{champion} icin {role} hazirligini kaydettim efendim. "
            "Run/build sayfalarini otomatik acmiyorum; yanlis komutlarda tarayici acilmasin diye bu akis kapatildi."
        )
    ...
    def check_warnings(self, command, current_time=None):
        ...
        if any(word in words for word in ["sil", "sileceksin", "silinsin", "silme"]) and not any(word in clean for word in ["onayliyorum", "bunu yap", "eminim"]):
    ...
    def execute_pending(self):
        ...
        if action["type"] == "shutdown":
            ...
            silent_run(["shutdown", "/s", "/t", str(delay_seconds)], capture_output=True, text=True, timeout=5)
            ...
            return "DONE"
            silent_run(["shutdown", "/s", "/t", "10"], capture_output=True, text=True, timeout=5)
            self.speak("Bilgisayar 10 saniye içinde kapatılıyor.")
            return "DONE"

        if action["type"] == "restart":
            silent_run(["shutdown", "/r", "/t", "10"], capture_output=True, text=True, timeout=5)
        ...
        if action["type"] == "incognito_edge":
            ...
            if url:
                ...
                silent_shell("start msedge -inprivate")
        ...
        if action["type"] == "edge_research":
            ...
            silent_shell(f'start msedge -inprivate "{youtube_url}"')
            time.sleep(0.7)
            silent_shell(f'start msedge -inprivate "{google_url}"')
        ...
        if action["type"] == "edge_site":
            silent_shell(f'start msedge "{action["url"]}"')
    ...
    def start_riot_client(self):
        ...
        if riot_path.exists():
            silent_popen([str(riot_path)])
        else:
            silent_popen([r"C:\Riot Games\Riot Client\RiotClientServices.exe"])

    def open_discord(self):
        if self.is_process_running_contains("discord"):
            self.log_event("DISCORD", "Zaten acik")
            return "Discord zaten acik efendim."
        ...
        for path in paths:
            ...
            try:
                if target.lower().endswith("update.exe"):
                    silent_popen([target, "--processStart", "Discord.exe"])
                else:
                    silent_popen([target])
                self.log_event("DISCORD", "Acildi")
                return "Discord acildi efendim."
            except Exception as exc:
                self.log_event("HATA", f"Discord acilamadi: {exc}")

        self.log_event("DISCORD", "Bulunamadi")
        return "Discord bulunamadi efendim. Yuklu mu kontrol edin."
    ...
    def close_discord(self):
        ...
        if psutil is not None:
            try:
                ...
                self.log.emit(f"> DISCORD: psutil ile kapatma denenemedi: {exc}")
        ...
        if not closed:
            try:
                ...
                self.log.emit(f"> DISCORD: taskkill denenemedi: {exc}")

        self.log_event("DISCORD", "Kapatildi" if closed else "Kapatilacak surec bulunamadi")
        return "Discord kapatildi efendim." if closed else "Discord zaten kapali gorunuyor efendim."
    ...
    def is_process_running_contains(self, needle):
        ...
        try:
            ...
            return needle in norm(completed.stdout or "")
        except Exception:
            return False
        return False
    ...
    def parse_discord_message_command(self, text):
        ...
        if not raw:
            return None
        match = re.match(r"^(^:jarvis\s+)^(^:discordda|discord da|discorddan|discord dan|dc de|dcde|dcden|dc den)\s+(.+)$", raw, flags=re.I)
        if not match:
            return None
        ...
        while True:
            ...
            if not words:
                return None
            last_word = norm(words[-1])
            if last_word not in {norm(word) for word in son_kelimeler}:
                break
            mesaj = " ".join(words[:-1]).strip()
    ...
    def discord_send_message(self, kisi_adi, mesaj):
        ...
        self.last_action_summary = f"Discord mesaji hazirlandi ama gonderilmedi: {kisi_adi}"
        self.log_event("DISCORD", f"{kisi_adi} e mesaj gonderme otomasyonu guvenilir olmadigi icin durduruldu")
        return (
            "Efendim, Discord mesaj gönderme özelliği "
            "şu an güvenilir çalışmıyor. "
            "Lütfen mesajı kendiniz gönderin."
        )

    def start_spotify(self):
        silent_shell("start spotify")
    ...
    def run_game_mode_csgo(self):
        ...
        open_uri_silent("steam://rungameid/730")
    ...
    def computer_command(self, text):
        ...
        restart_requested = (
            any(phrase in text for phrase in ["jarvisi yeniden baslat", "jarvis yeniden baslat", "jarvisi restart", "jarvis restart", "kendini yeniden baslat", "kendini restart"])
            or (
        ...
        if restart_requested:
            return "Efendim, yeniden başlatma özelliği şimdilik kaldırıldı."
        ...
        if any(phrase in text for phrase in ["discord", "dc de", "dcde", "dcden", "dc den"]) and any(
            ...
            return "Efendim, mesaj düzenleme özelliği henüz desteklenmiyor. Yeni mesaj göndereyim mi^"
        ...
        if text in ["iptal et", "iptal ediyorum", "vazgec", "kapatmayi iptal et", "kapanmayi iptal et"]:
            ...
            silent_run(["shutdown", "/a"], capture_output=True, text=True, timeout=5)
        ...
        if any(word in text for word in ["virus", "antivirus", "defender", "tehdit", "zararli"]):
            if any(phrase in text for phrase in ["windows guvenligi ac", "defender ac", "antivirus ac"]):
                open_uri_silent("windowsdefender:")
        ...
        restart_phrase = "yeniden baslat" in text or "restart" in text
        restart_target = any(target in text for target in ["bilgisayari", "pc", "windows", "sistemi"])

        if restart_phrase and not restart_target:
            return "Efendim, yeniden başlatma özelliği şimdilik kaldırıldı."

        if restart_phrase and restart_target:
        ...
        if "edge" in text and "ac" in text:
            silent_shell("start msedge")
            return "Edge açıldı."

        if any(phrase in text for phrase in ["discord kapat", "discordu kapat", "dc kapat", "dc yi kapat"]):
            return self.close_discord()
        ...
        if spotify_requested:
            if "kapat" in text:
                silent_run(["taskkill", "/f", "/im", "Spotify.exe"], capture_output=True, text=True, timeout=5)
...
class JarvisUI(QWidget):
    guvenlik_uyarisi = pyqtSignal(str)
    ...
    def __init__(self, start_mini=False, start_silent=False):
        ...
        self.file_btn = QPushButton("\U0001F4CE", self)
        ...
        self.chat_min_btn = QPushButton("-", self)
        self.chat_expand_btn = QPushButton("[ ]", self)
        self.chat_close_btn = QPushButton("X", self)
        self.chat_min_btn.clicked.connect(self.enter_mini_mode)
        self.chat_expand_btn.clicked.connect(self.toggle_chat_fullscreen)
        self.chat_close_btn.clicked.connect(self.close_chat_panel)

        self.listen_mode_btn = QPushButton("DINLEME MODU", self)
        self.listen_mode_btn.clicked.connect(self.listen_once)
        ...
        for button in [
            ...
            self.close_btn,
            self.chat_min_btn,
            self.chat_expand_btn,
            self.chat_close_btn,
            self.listen_mode_btn,
        ]:
            button.setCursor(Qt.CursorShape.PointingHandCursor)
            button.setStyleSheet("""
            QPushButton {
                    background: rgba(0, 44, 18, 210);
                    color: #baffc9;
                    border: 1px solid #00ff66;
                    border-radius: 5px;
                    font-family: Consolas;
        ...
        self.core.security_alert.connect(self.guvenlik_uyarisi.emit)
        self.guvenlik_uyarisi.connect(self.guvenlik_uyari_goster)
        ...
        QTimer.singleShot(3000, self.start_watchdog)
        QTimer.singleShot(4000, self.start_program_monitor)
        QTimer.singleShot(5000, self.start_network_monitor)

    def start_watchdog(self):
        self.core.start_watchdog()

    def start_program_monitor(self):
        self.core.log_event("INFO", "Program monitor gecikmeli baslatildi")

    def start_network_monitor(self):
        self.core.log_event("INFO", "Network monitor gecikmeli baslatildi")
    ...
    def check_plan_reminders(self):
        now = dt.datetime.now()
        current_time = now.strftime("%H:%M")
        today_key = now.strftime("%Y-%m-%d")
        for item in self.core.get_plan_items("bugun"):
            hour = str(item.get("hour") or "").zfill(2)
            minute = str(item.get("minute") or "00").zfill(2)
            if not hour.strip():
                continue
            item_time = f"{hour}:{minute}"
            reminder_key = f"{today_key}|{item_time}|{item.get('title', '')}"
            if item_time != current_time or reminder_key in self.notified_plan_items:
                continue
            self.notified_plan_items.add(reminder_key)
            title = item.get("title") or "Plan hatirlaticisi"
            message = f"Efendim, saat {item_time}. Hatırlatma: {title}"
            self.bildirim_goster("JARVIS Hatırlatma", message)
            self.core.speak(message, force=True)
    ...
    def update_system_stats(self):
        try:
            ...
            self.gpu_percent = max(0.0, min(100.0, 12.0 + (cpu * 0.35) + (self.voice_energy * 20)))
            try:
                usage = shutil.disk_usage(str(BASE_DIR.anchor or "C:\\"))
                self.storage_percent = (usage.used / usage.total) * 100
            except Exception:
                self.storage_percent = 0.0
            self.cpu_temp_text = "--"
            if psutil is not None and hasattr(psutil, "sensors_temperatures"):
                try:
                    ...
                    flat = [item.current for values in temps.values() for item in values if getattr(item, "current", None)]
                    if flat:
                        self.cpu_temp_text = f"{max(flat):.0f} °C"
                except Exception:
                    self.cpu_temp_text = "--"
            if psutil is not None:
                try:
                    ...
                    if self._last_net_sample:
                        last_time, last_sent, last_recv = self._last_net_sample
                        elapsed = max(0.5, now - last_time)
                        down = max(0.0, (counters.bytes_recv - last_recv) * 8 / elapsed / 1_000_000)
                        up = max(0.0, (counters.bytes_sent - last_sent) * 8 / elapsed / 1_000_000)
                        self.net_speed_text = f"v {down:.1f} Mbps   ^ {up:.1f} Mbps"
                    self._last_net_sample = (now, counters.bytes_sent, counters.bytes_recv)
                except Exception:
                    self.net_speed_text = "v -- Mbps   ^ -- Mbps"
    ...
    def v4_rects(self, w, h):
        top_h = 42
        pad = max(24, int(w * 0.035))
        side_w = min(210, max(180, int(w * 0.19)))
        upper_y = top_h + 18
        upper_h = max(185, int(h * 0.25))
        chat_y = upper_y + upper_h + 20
        chat_h = max(150, min(int(h * 0.24), int(h * 0.28)))
        input_y = chat_y + chat_h + 14
        input_h = 42
        footer_h = 44
        footer_y = h - footer_h - 12
        lower_y = input_y + input_h + 22
        lower_h = max(80, footer_y - lower_y - 16)
        return {
            "top_h": top_h,
            "pad": pad,
            "side_w": side_w,
            "upper_y": upper_y,
            "upper_h": upper_h,
            "left_panel": QRectF(pad, upper_y, side_w, upper_h),
            "center_panel": QRectF(pad + side_w + 22, upper_y, w - (pad + side_w + 22) * 2, upper_h),
            "right_panel": QRectF(w - pad - side_w, upper_y, side_w, upper_h),
            "chat": QRectF(pad, chat_y, w - pad * 2, chat_h),
            "input_y": input_y,
            "input_h": input_h,
            "lower_y": lower_y,
            "lower_h": lower_h,
    ...
    def set_chat_style(self, fullscreen=False):
        font_size = 16 if fullscreen else 13
        self.chat.setStyleSheet(f"""
            QTextEdit {{
                background: rgba(0, 8, 2, 135);
                color: #00ff66;
                border: none;
                border-radius: 4px;
                font-family: Consolas;
    ...
    def toggle_chat_fullscreen(self):
    ...
    def close_chat_panel(self):
        if self.chat_fullscreen:
            ...
            return
        self.add_log("> SISTEM: Sohbet paneli aktif kalir.")

    def apply_layout(self):
        if self.mini_mode:
            for widget in [
                self.chat, self.input, self.send_btn, self.mic_btn, self.wake_btn,
                self.mute_btn, self.file_btn, self.min_btn, self.max_btn, self.close_btn,
                self.chat_min_btn, self.chat_expand_btn, self.chat_close_btn, self.listen_mode_btn,
            ]:
                widget.hide()
            self.size_grip.hide()
            return
        ...
        for widget in [
            self.chat, self.input, self.send_btn, self.mic_btn, self.file_btn,
            self.min_btn, self.max_btn, self.close_btn, self.chat_min_btn,
            self.chat_expand_btn, self.chat_close_btn, self.listen_mode_btn,
        ]:
            widget.show()
        self.wake_btn.hide()
        self.mute_btn.hide()
        ...
        w = self.width()
        h = self.height()
        self.size_grip.setGeometry(w - 18, h - 18, 18, 18)

        if self.cockpit_mode:
            for widget in [self.chat_min_btn, self.chat_expand_btn, self.chat_close_btn, self.listen_mode_btn]:
                widget.hide()
            self.size_grip.hide()
        ...
        self.min_btn.setGeometry(w - 86, 10, 22, 22)
        self.max_btn.setGeometry(w - 58, 10, 22, 22)
        self.close_btn.setGeometry(w - 30, 10, 22, 22)

        if self.chat_fullscreen:
            for widget in [self.input, self.send_btn, self.mic_btn, self.file_btn, self.listen_mode_btn]:
                widget.hide()
            self.set_chat_style(fullscreen=True)
            self.chat.setGeometry(70, 92, w - 140, h - 150)
            self.chat_min_btn.hide()
            self.chat_close_btn.hide()
            self.chat_expand_btn.show()
            self.chat_expand_btn.setText("< GERI DON")
            self.chat_expand_btn.setGeometry(w - 168, 54, 130, 30)
            return

        self.set_chat_style(fullscreen=False)
        self.chat_expand_btn.setText("[ ]")
        rects = self.v4_rects(w, h)
        chat = rects["chat"]
        self.chat.setGeometry(int(chat.x() + 16), int(chat.y() + 36), int(chat.width() - 32), int(chat.height() - 48))
        self.chat_min_btn.setGeometry(int(chat.right() - 92), int(chat.y() + 7), 22, 22)
        self.chat_expand_btn.setGeometry(int(chat.right() - 64), int(chat.y() + 7), 22, 22)
        self.chat_close_btn.setGeometry(int(chat.right() - 36), int(chat.y() + 7), 22, 22)

        pad = rects["pad"]
        send_w = 132
        attach = 44
        input_y = rects["input_y"]
        input_h = rects["input_h"]
        self.file_btn.setGeometry(pad, input_y, attach, input_h)
        self.input.setGeometry(pad + attach + 12, input_y, w - pad * 2 - attach - send_w - 28, input_h)
        self.send_btn.setGeometry(w - pad - send_w, input_y, send_w, input_h)

        mic_size = 100
        mic_x = int((w - mic_size) / 2)
        mic_y = int(rects["lower_y"] + max(6, (rects["lower_h"] - mic_size) / 2) - 8)
        self.mic_btn.setGeometry(mic_x, mic_y, mic_size, mic_size)
        self.listen_mode_btn.setGeometry(mic_x - 10, mic_y + mic_size + 8, mic_size + 20, 26)
        # ESKİ > güncellendi
    ...
    def show_message(self, mesaj):
        self.add_log(f"> JARVIS: {mesaj}")

    def guvenlik_uyari_goster(self, mesaj):
        self.add_log(f"> GUVENLIK: {mesaj}")
        threading.Thread(target=self.core.speak, args=(mesaj,), kwargs={"force": True}, daemon=True).start()
    ...
    def default_panel_geometry(self):
        ...
        width = min(1100, max(900, int(screen.width() * 0.72)))
        height = min(750, max(650, int(screen.height() * 0.82)))
        x = screen.left() + max(12, int((screen.width() - width) / 2))
        y = screen.top() + max(12, int((screen.height() - height) / 2))
    ...
    def leave_mini_mode(self):
        ...
        if self.restore_cockpit_after_mini:
            ...
            return
        self.cockpit_mode = False
        target = self.normal_geometry or self.geometry()
        self.setMinimumSize(900, 650)
        self.setMaximumSize(16777215, 16777215)
        self.resize(max(900, target.width()), max(650, target.height()))
    ...
    def leave_cockpit_mode(self):
        self.cockpit_mode = False
        self.expanded = False
        target = self.pre_cockpit_geometry or self.normal_geometry or self.geometry()
        self.setMinimumSize(900, 650)
        self.setMaximumSize(16777215, 16777215)
        self.resize(max(900, target.width()), max(650, target.height()))
        self.move(target.topLeft())
    ...
    def paintEvent(self, event):
        ...
        self.draw_v4_dashboard(painter, w, h, green)
        return

    def draw_v4_dashboard(self, painter, w, h, green):
        rects = self.v4_rects(w, h)
        bg = QRectF(0, 0, w - 1, h - 1)
        path = QPainterPath()
        path.addRoundedRect(bg, 10, 10)
        painter.fillPath(path, QColor(0, 8, 0, 248))
        gradient = QLinearGradient(0, 0, 0, h)
        gradient.setColorAt(0, QColor(0, 28, 12, 95))
        gradient.setColorAt(0.45, QColor(0, 4, 0, 248))
        gradient.setColorAt(1, QColor(0, 38, 14, 110))
        painter.fillPath(path, gradient)

        painter.setPen(QPen(QColor(0, 255, 102, 145), 1))
        painter.drawPath(path)
        self.draw_matrix_rain(painter, w, h, green)
        painter.fillRect(QRectF(0, 0, w, h), QColor(0, 0, 0, 50))

        self.draw_v4_top_bar(painter, w, rects)
        if self.chat_fullscreen:
            self.draw_v4_chat_fullscreen(painter, w, h)
            return
        self.draw_v4_system_panel(painter, rects["left_panel"])
        self.draw_v4_center_panel(painter, rects["center_panel"])
        self.draw_v4_info_panel(painter, rects["right_panel"])
        self.draw_v4_chat_frame(painter, rects["chat"])
        self.draw_v4_input_frame(painter, w, rects)
        self.draw_v4_quick_panel(painter, QRectF(rects["pad"], rects["lower_y"], rects["side_w"], rects["lower_h"]))
        self.draw_v4_mic_area(painter, w, rects)
        self.draw_v4_recent_panel(painter, QRectF(w - rects["pad"] - rects["side_w"], rects["lower_y"], rects["side_w"], rects["lower_h"]))
        self.draw_v4_footer(painter, w, h, rects)

    def v4_box(self, painter, rect, title="", cut=14, alpha=165):
        ...
        path.lineTo(x, y + cut)
        path.closeSubpath()
        painter.fillPath(path, QColor(0, 8, 3, alpha))
        painter.setPen(QPen(QColor(0, 255, 102, 70), 4))
        painter.drawPath(path)
        painter.setPen(QPen(QColor(0, 255, 102, 205), 1))
        painter.drawPath(path)
        if title:
            painter.setFont(QFont("Consolas", 10, QFont.Weight.Bold))
            painter.setPen(QColor(0, 255, 102, 235))
            painter.drawText(QRectF(x + 18, y + 10, rw - 36, 18), Qt.AlignmentFlag.AlignLeft, title)

    def draw_v4_top_bar(self, painter, w, rects):
        top_h = rects["top_h"]
        painter.fillRect(QRectF(0, 0, w, top_h), QColor(0, 8, 3, 170))
        painter.setPen(QPen(QColor(0, 255, 102, 110), 1))
        painter.drawLine(8, top_h - 1, w - 8, top_h - 1)
        painter.setFont(QFont("Consolas", 10, QFont.Weight.Bold))
        painter.setPen(QColor(0, 255, 102, 235))
        painter.drawText(QRectF(24, 10, 180, 20), Qt.AlignmentFlag.AlignLeft, f"J.A.R.V.I.S {APP_VERSION}")
        painter.setPen(QPen(QColor(0, 255, 102, 120), 2))
        for i in range(5):
            x = 190 + i * 12
            painter.drawLine(x, 15, x + 7, 15)
            painter.drawLine(x, 22, x + 7, 22)

    def draw_v4_system_panel(self, painter, rect):
        self.v4_box(painter, rect, "SISTEM BILGILERI")
        items = [
            ("CPU", self.cpu_percent),
            ("RAM", self.ram_percent),
            ("GPU", self.gpu_percent),
            ("DEPOLAMA", self.storage_percent),
        ]
        y = rect.y() + 42
        for label, value in items:
            painter.setFont(QFont("Consolas", 8, QFont.Weight.Bold))
            painter.setPen(QColor(0, 255, 102, 230))
            painter.drawText(QRectF(rect.x() + 22, y, 76, 14), Qt.AlignmentFlag.AlignLeft, label)
            painter.setPen(QColor(155, 255, 185, 230))
            painter.drawText(QRectF(rect.x() + 22, y + 14, 52, 14), Qt.AlignmentFlag.AlignLeft, f"%{value:.0f}")
            bar = QRectF(rect.x() + 92, y + 18, rect.width() - 122, 5)
            painter.fillRect(bar, QColor(0, 40, 14, 160))
            painter.fillRect(QRectF(bar.x(), bar.y(), bar.width() * max(0, min(100, value)) / 100, bar.height()), QColor(0, 255, 102, 220))
            y += 36

    def draw_v4_center_panel(self, painter, rect):
        cx = rect.center().x()
        radar_cy = rect.y() + 46
        pulse = math.sin(self.pulse * 0.08)
        painter.setPen(QPen(QColor(0, 255, 102, 45), 9))
        painter.drawEllipse(QPoint(int(cx), int(radar_cy)), 46, 46)
        for radius, alpha in [(28, 140), (39, 100), (52, 70), (64, 45)]:
            painter.setPen(QPen(QColor(0, 255, 102, alpha), 1))
            painter.drawEllipse(QPoint(int(cx), int(radar_cy)), radius, radius)
        painter.setPen(QPen(QColor(0, 255, 102, 170), 2))
        arc_rect = QRectF(cx - 48, radar_cy - 48, 96, 96)
        painter.drawArc(arc_rect, int((self.pulse * 4) % 360) * 16, 115 * 16)
        tri = QPainterPath()
        tri.moveTo(cx, radar_cy - 20)
        tri.lineTo(cx - 22, radar_cy + 18)
        tri.lineTo(cx + 22, radar_cy + 18)
        tri.closeSubpath()
        painter.fillPath(tri, QColor(0, 255, 102, 25))
        painter.setPen(QPen(QColor(0, 255, 102, 235), 2))
        painter.drawPath(tri)
        painter.drawEllipse(QPoint(int(cx), int(radar_cy)), int(14 + pulse * 3), int(14 + pulse * 3))

        title = "J.A.R.V.I.S"
        font = QFont("Arial Black", 48, QFont.Weight.Black)
        font.setLetterSpacing(QFont.SpacingType.AbsoluteSpacing, 4)
        while QFontMetrics(font).horizontalAdvance(title) > rect.width() - 40 and font.pointSize() > 34:
            font.setPointSize(font.pointSize() - 1)
        painter.setFont(font)
        title_y = 80
        status_y = 140
        title_rect = QRectF(rect.x(), title_y, rect.width(), 56)
        for width, alpha in [(12, 28), (7, 70), (3, 145)]:
            painter.setPen(QPen(QColor(0, 255, 102, alpha), width))
            painter.drawText(title_rect, Qt.AlignmentFlag.AlignCenter, title)
        painter.setPen(QColor(0, 255, 102, 245))
        painter.drawText(title_rect, Qt.AlignmentFlag.AlignCenter, title)
        painter.setFont(QFont("Consolas", 9, QFont.Weight.Bold))
        painter.setPen(QColor(110, 255, 150, 170))
        painter.drawText(QRectF(rect.x(), 166, rect.width(), 18), Qt.AlignmentFlag.AlignCenter, "JUST A RATHER VERY INTELLIGENT SYSTEM")
        status_text = f"DURUM: {self.current_status}"
        status_width = min(rect.width() - 40, max(170, QFontMetrics(painter.font()).horizontalAdvance(status_text) + 34))
        status_rect = QRectF(cx - status_width / 2, status_y, status_width, 24)
        self.v4_box(painter, status_rect, "", cut=10, alpha=135)
        painter.setPen(QColor(0, 255, 102, 235))
        painter.drawText(QRectF(status_rect.x(), status_y + 3, status_rect.width(), 18), Qt.AlignmentFlag.AlignCenter, status_text)
        return

    def draw_v4_info_panel(self, painter, rect):
        self.v4_box(painter, rect)
        items = [
            ("SAAT", time.strftime("%H:%M:%S")),
            ("TARIH", dt.datetime.now().strftime("%d.%m.%Y")),
            ("BAGLANTI", "AKTIF"),
            ("GUC MODU", "DENGELI"),
        ]
        y = rect.y() + 28
        for label, value in items:
            painter.setFont(QFont("Consolas", 8, QFont.Weight.Bold))
            painter.setPen(QColor(0, 255, 102, 230))
            painter.drawText(QRectF(rect.x() + 24, y, rect.width() - 48, 14), Qt.AlignmentFlag.AlignLeft, label)
            painter.setFont(QFont("Consolas", 10 if label == "SAAT" else 8, QFont.Weight.Bold))
            painter.setPen(QColor(175, 255, 195, 235))
            painter.drawText(QRectF(rect.x() + 24, y + 15, rect.width() - 48, 18), Qt.AlignmentFlag.AlignLeft, value)
            y += 39

    def draw_v4_chat_frame(self, painter, rect):
        self.v4_box(painter, rect, "JARVIS SOHBET", cut=18, alpha=178)
        painter.setPen(QPen(QColor(0, 255, 102, 90), 1))
        painter.drawLine(int(rect.x() + 12), int(rect.y() + 30), int(rect.right() - 12), int(rect.y() + 30))

    def draw_v4_chat_fullscreen(self, painter, w, h):
        self.v4_box(painter, QRectF(45, 48, w - 90, h - 74), "JARVIS SOHBET -- TAM EKRAN", cut=20, alpha=185)

    def draw_v4_input_frame(self, painter, w, rects):
        y = rects["input_y"]
        painter.setPen(QPen(QColor(0, 255, 102, 110), 1))
        painter.drawLine(rects["pad"], y - 8, w - rects["pad"], y - 8)

    def draw_v4_quick_panel(self, painter, rect):
        self.v4_box(painter, rect, "HIZLI KOMUTLAR")
        items = ["UYGULAMALAR", "DOSYALAR", "AYARLAR", "WEB'DE ARA", "EKRAN GORUNTUSU"]
        y = rect.y() + 42
        painter.setFont(QFont("Consolas", 8, QFont.Weight.Bold))
        for item in items:
            painter.setPen(QColor(0, 255, 102, 220))
            painter.drawText(QRectF(rect.x() + 22, y, rect.width() - 46, 18), Qt.AlignmentFlag.AlignLeft, item)
            painter.drawText(QRectF(rect.right() - 24, y, 12, 18), Qt.AlignmentFlag.AlignCenter, ">")
            y += 25

    def draw_v4_mic_area(self, painter, w, rects):
        lower_y = rects["lower_y"]
        lower_h = rects["lower_h"]
        cx = w / 2
        cy = lower_y + lower_h / 2 - 8
        painter.setPen(QPen(QColor(0, 255, 102, 125), 2))
        last_left = None
        last_right = None
        for i in range(95):
            amp = (math.sin(i * 0.37 + self.pulse * 0.22) + math.sin(i * 0.91 + self.pulse * 0.12)) * (10 + self.voice_energy * 28)
            x_left = cx - 70 - i * 3
            x_right = cx + 70 + i * 3
            p_left = QPoint(int(x_left), int(cy + amp))
            p_right = QPoint(int(x_right), int(cy - amp))
            if last_left:
                painter.drawLine(last_left, p_left)
                painter.drawLine(last_right, p_right)
            last_left, last_right = p_left, p_right
        for radius, alpha, width in [(54, 60, 10), (68, 90, 2), (84, 55, 1)]:
            painter.setPen(QPen(QColor(0, 255, 102, alpha), width))
            painter.drawEllipse(QPoint(int(cx), int(cy)), radius, radius)
        painter.setPen(QPen(QColor(0, 255, 102, 235), 5))
        painter.drawRoundedRect(QRectF(cx - 14, cy - 28, 28, 46), 12, 12)
        painter.drawLine(int(cx), int(cy + 18), int(cx), int(cy + 38))
        painter.drawLine(int(cx - 22), int(cy + 38), int(cx + 22), int(cy + 38))

    def draw_v4_recent_panel(self, painter, rect):
        self.v4_box(painter, rect, "SON KOMUTLAR")
        commands = self.recent_commands(5)
        y = rect.y() + 42
        painter.setFont(QFont("Consolas", 8, QFont.Weight.Bold))
        for command in commands:
            clean = str(command).replace("\n", " ")[:24]
            painter.setPen(QColor(0, 255, 102, 210))
            painter.drawText(QRectF(rect.x() + 18, y, rect.width() - 30, 18), Qt.AlignmentFlag.AlignLeft, f"- {clean}")
            y += 24

    def recent_commands(self, limit=5):
        try:
            history = self.core.brain.memory.get("command_history", [])
            return [item.get("command", "") for item in history[-limit:]][::-1] or ["Komut yok"]
        except Exception:
            return ["Komut yok"]

    def draw_v4_footer(self, painter, w, h, rects):
        y = rects["footer_y"]
        margin = rects["pad"]
        gap = 12
        box_w = (w - margin * 2 - gap * 3) / 4
        items = [
            ("SISTEM KORUMASI", "AKTIF"),
            ("PERFORMANS", "IYI"),
            ("SICAKLIK", self.cpu_temp_text),
            ("AG HIZI", self.net_speed_text),
        ]
        for index, (title, value) in enumerate(items):
            rect = QRectF(margin + index * (box_w + gap), y, box_w, rects["footer_h"])
            self.v4_box(painter, rect, "", cut=10, alpha=160)
            painter.setFont(QFont("Consolas", 8, QFont.Weight.Bold))
            painter.setPen(QColor(0, 255, 102, 220))
            painter.drawText(QRectF(rect.x() + 18, rect.y() + 8, rect.width() - 34, 12), Qt.AlignmentFlag.AlignLeft, title)
            painter.setPen(QColor(170, 255, 190, 230))
            painter.drawText(QRectF(rect.x() + 18, rect.y() + 22, rect.width() - 34, 14), Qt.AlignmentFlag.AlignLeft, value)
    ...
    def draw_title(self, painter, w, h, green):
        text = "J.A.R.V.I.S"
        size = max(34, int(w * 0.128)) + int(self.voice_energy * 2)
        y = int(h * 0.215 + self.title_jitter)
        families = set(QFontDatabase.families())
        family = next((name for name in ["Orion", "Impact", "Arial Black"] if name in families), "Arial Black")
        font = QFont(family, size, QFont.Weight.Black)
        font.setStyleStrategy(QFont.StyleStrategy.PreferAntialias)
        font.setLetterSpacing(QFont.SpacingType.AbsoluteSpacing, 3)
        while QFontMetrics(font).horizontalAdvance(text) > int(w * 0.82) and size > 28:
            size -= 1
            font = QFont(family, size, QFont.Weight.Black)
            font.setStyleStrategy(QFont.StyleStrategy.PreferAntialias)
            font.setLetterSpacing(QFont.SpacingType.AbsoluteSpacing, 3)
        painter.setFont(font)
        text_rect = QRectF(0, y - 6, w, 82)
        glow_boost = int(self.voice_energy * 55)
        for dx, dy, alpha in [
            (-2, 0, 70),
            (2, 0, 70),
            (0, -2, 60),
            ...
            painter.setPen(QColor(0, 255, 102, min(255, alpha + glow_boost)))
            painter.drawText(text_rect.translated(dx, dy), Qt.AlignmentFlag.AlignCenter, text)
        for width, alpha in [(20, 18), (13, 35), (8, 80), (4, 150)]:
            painter.setPen(QPen(QColor(0, 255, 102, min(255, alpha + glow_boost)), width))
            painter.drawText(text_rect, Qt.AlignmentFlag.AlignCenter, text)
        painter.setPen(QColor(0, 255, 102, 250))
        painter.drawText(text_rect, Qt.AlignmentFlag.AlignCenter, text)
        painter.setPen(QPen(QColor(0, 5, 2, 95), 2))
    ...
    def draw_wave(self, painter, w, h, green):
        y = int(h * 0.79)
        wave_alpha = 145 + int(self.voice_energy * 110)
        painter.setPen(QPen(QColor(0, 255, 102, wave_alpha), 2 + int(self.voice_energy * 3)))
        last = None
        for x in range(10, w - 10, 3):
            ...
            height = int((1 - amp) * (28 + self.voice_energy * 48) + abs(speech) * 30)
        ...
        cx = w // 2
        radius = max(48, int(w * 0.112)) + int(self.voice_energy * 12)
        painter.setPen(QPen(QColor(0, 255, 102, 36), 13))
        painter.drawEllipse(QPoint(cx, y), radius + 12, radius + 12)
        painter.setPen(QPen(QColor(0, 255, 170, 90), 4))
        painter.drawEllipse(QPoint(cx, y), radius + 19, radius + 19)
        painter.setPen(QPen(QColor(0, 255, 102, 150), 2))
        painter.drawEllipse(QPoint(cx, y), radius + 26, radius + 26)
        painter.setPen(QPen(green, 3 + int(self.voice_energy * 2)))
        painter.drawEllipse(QPoint(cx, y), radius, radius)
        painter.setPen(QPen(QColor(0, 255, 102, 235), 4))
        mic_w = 20 + int(self.voice_energy * 5)
        mic_h = 38 + int(self.voice_energy * 9)
        painter.drawRoundedRect(QRectF(cx - mic_w / 2, y - mic_h / 2 - 2, mic_w, mic_h), 9, 9)
        painter.setPen(QPen(QColor(0, 255, 170, 220), 3))
        painter.drawArc(QRectF(cx - 24, y - 21, 48, 46), 200 * 16, 140 * 16)
        painter.drawLine(cx, int(y + mic_h / 2 - 2), cx, int(y + mic_h / 2 + 15))
        painter.drawLine(cx - 15, int(y + mic_h / 2 + 15), cx + 15, int(y + mic_h / 2 + 15))
        painter.setFont(QFont("Consolas", 7, QFont.Weight.Bold))
        painter.setPen(QColor(0, 255, 102, 235))
        painter.drawText(QRectF(cx - 54, y + radius - 12, 108, 12), Qt.AlignmentFlag.AlignCenter, "MIKROFON: ACIK")
        return
        painter.setPen(QPen(QColor(0, 255, 102, 155), 1))
        last = None
        for x in range(10, w - 10, 3):
        ...
        painter.drawText(QRectF(cx - 28, y - 24, 56, 56), Qt.AlignmentFlag.AlignCenter, "ğŸ™")
    ...
    def draw_footer(self, painter, w, h, green):
        ...
        def draw_icon(kind, cx, cy):
            painter.setPen(QPen(QColor(0, 255, 102, 210), 1.4))
            painter.setBrush(Qt.BrushStyle.NoBrush)
            if kind == "shield":
                shield = QPainterPath()
                shield.moveTo(cx, cy - 9)
                shield.lineTo(cx + 9, cy - 5)
                shield.lineTo(cx + 7, cy + 6)
                shield.lineTo(cx, cy + 12)
                shield.lineTo(cx - 7, cy + 6)
                shield.lineTo(cx - 9, cy - 5)
                shield.closeSubpath()
                painter.drawPath(shield)
                painter.drawLine(int(cx - 4), int(cy), int(cx - 1), int(cy + 4))
                painter.drawLine(int(cx - 1), int(cy + 4), int(cx + 5), int(cy - 4))
            elif kind == "globe":
                painter.drawEllipse(QPoint(int(cx), int(cy)), 10, 10)
                painter.drawLine(int(cx - 10), int(cy), int(cx + 10), int(cy))
                painter.drawEllipse(QPoint(int(cx), int(cy)), 4, 10)
            elif kind == "chip":
                painter.drawRoundedRect(QRectF(cx - 8, cy - 8, 16, 16), 2, 2)
                for offset in [-10, -4, 4, 10]:
                    painter.drawLine(int(cx + offset), int(cy - 12), int(cx + offset), int(cy - 9))
                    painter.drawLine(int(cx + offset), int(cy + 9), int(cx + offset), int(cy + 12))
                    painter.drawLine(int(cx - 12), int(cy + offset), int(cx - 9), int(cy + offset))
                    painter.drawLine(int(cx + 9), int(cy + offset), int(cx + 12), int(cy + offset))
            else:
                painter.drawEllipse(QPoint(int(cx), int(cy)), 10, 10)
                painter.drawLine(int(cx), int(cy), int(cx), int(cy - 6))
                painter.drawLine(int(cx), int(cy), int(cx + 5), int(cy + 4))

        def footer_box(index, icon, title, sub):
            x = margin + index * (box_w + gap)
            rect = QRectF(x, y, box_w, bar_h)
            painter.fillRect(rect, QColor(0, 8, 3, 165))
            painter.setPen(QPen(QColor(0, 255, 102, 82), 3))
            painter.drawRoundedRect(rect, 4, 4)
            painter.setPen(QPen(QColor(0, 255, 102, 190), 1))
            painter.drawRoundedRect(rect.adjusted(1, 1, -1, -1), 4, 4)
            draw_icon(icon, x + 19, y + bar_h / 2)
            painter.setFont(QFont("Consolas", 7, QFont.Weight.Bold))
            painter.setPen(QColor(0, 255, 102, 235))
            painter.drawText(QRectF(x + 38, y + 5, box_w - 42, 10), Qt.AlignmentFlag.AlignLeft, title)
            painter.setFont(QFont("Consolas", 6, QFont.Weight.Bold))
            painter.setPen(QColor(120, 255, 170, 220))
            painter.drawText(QRectF(x + 38, y + 17, box_w - 42, 10), Qt.AlignmentFlag.AlignLeft, sub)

        footer_box(0, "shield", "GUVENLIK", "SISTEM KORUNUYOR")
        footer_box(1, "globe", "BAGLANTI", "INTERNET: AKTIF")
        footer_box(2, "chip", "KAYNAK", self.system_stats_text)
        footer_box(3, "clock", "SAAT", time.strftime("%H:%M:%S"))
...
def main():
    ...
    mutex = ctypes.windll.kernel32.CreateMutexW(None, False, "Global\\JARVIS_EFE_SINGLE_INSTANCE")
